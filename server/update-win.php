...
if(!isset($_GET['magic']) || !isset($_GET['winner']) || !isset($_GET['user']) || $_GET['magic'] != "NechAtHa6RuzeR8x") {
    echo '<connect4 status="no" msg="magic" />';
    exit;
}

process($_GET['user'], $_GET['winner']);

function process($username, $winner){
    $pdo = pdo_connect();
    ...
    $query = "UPDATE connect4game SET winner='$winner'";
    $pdo->query($query);
